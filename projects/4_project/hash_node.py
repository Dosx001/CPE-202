...
class HashTable:
    ...
    def insert(self, key, value):
        ...
        if self.hash_table[loc] == None:
            self.hash_table[loc] = Node(key, value)
            self.num_items += 1
        elif key == self.hash_table[loc].key:
            self.hash_table[loc].add_value(value)
        else:
        ...
        if .5 < self.num_items / self.table_size:
            self.table_size *= 2
            self.table_size += 1
            storage = set(self.hash_table)
            storage.remove(None)
            self.hash_table = [None] * self.table_size
            self.num_items=0
            for key_value in storage:
                for value in key_value.values:
                    self.insert(key_value.key, value)
               # loc = self.horner_hash(key_value.key)
               # if self.hash_table[loc] == None:
               #     self.hash_table[loc] == key_value
               # else:
               #     self.quad_probe2(key_value, loc)
               # self.num_items += 1
    ...
    def in_table(self, key):
        ...
        if self.hash_table[loc] == None:
            return False
        if self.hash_table[loc].key == key:
           return True
    ...
    def get_all_keys(self):
        ...
        return [i.key for i in self.hash_table if i != None]
    ...
    def get_value(self, key):
        ...
        if self.hash_table[loc] == None:
            return None
        if self.hash_table[loc].key == key:
            return sorted(list(self.hash_table[loc].values))
        loc = self.quad_key_finder(key, loc)
        if loc == None:
            return None
        return sorted(list(self.hash_table[loc].values))
    ...
    def quad_probe(self, key, value, loc, n = 1):
        ...
        if self.hash_table[loc_q] == None:
            self.hash_table[loc_q] = Node(key, value)
            return True
        if self.hash_table[loc_q].key == key:
            self.hash_table[loc_q].add_value(value)
            return False
        n += 1
        return self.quad_probe(key, value, loc, n)

    def quad_probe_2(self, key_value, loc, n = 1):
        loc_q = (loc + n ** 2) % self.table_size
        if self.hash_table[loc_q] == None:
            self.hash_table[loc_q] = key_value
            return 
        n += 1
        return self.quad_probe(key, value, loc, n)
